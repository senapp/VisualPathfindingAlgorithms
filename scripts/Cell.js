function Cell(i, j) {
  ...
  this.show = function (fill = "white") {
    ...
    else if (renderMode == 1) ctx.arc(i * w + w / 2, j * h + h / 2, (w + h) / 4, 0, Math.PI * 2);
    else if (renderMode == 2 && this.wall) ctx.arc(i * w + w / 2, j * h + h / 2, (w + h) / 8, 0, Math.PI * 2);

    if (fill && renderMode != 2 || this.wall) {
      ctx.fillStyle = this.wall ? "black" : fill;
      ctx.fill();
    }
    ctx.stroke();
  };
